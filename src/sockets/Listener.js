...
class Listener
{
     ...
     */
    constructor ( handle, httpsServer = null )
    {
    ...
    open ()
    {
        ...
        this.logger.debug( `listener opening` );

        try
        {
            // Configurar el WebSocket Server
            const wsOptions = {
                verifyClient: this.verifyClient.bind( this )
            };

            // Verificar qué opciones tenemos disponibles
            this.logger.inform( 'Settings:', JSON.stringify( {
                hasServer: !!this.settings.server,
                listeningPort: this.settings.listeningPort
            } ) );

            // Configurar el WebSocket server
            if ( this.httpsServer )
            {
                wsOptions.server = this.httpsServer;
                this.logger.inform( "WebSocket using HTTPS server" );
            } else if ( this.settings.server ) {
                wsOptions.server = this.settings.server;
                this.logger.inform( "WebSocket using HTTP server" );
            }
            else
            {
                wsOptions.port = this.settings.listeningPort || 443;
                this.logger.inform( `WebSocket using port ${ wsOptions.port }` );
            }

            // Mostrar las opciones que vamos a usar
            this.logger.inform( 'WS Options:', wsOptions );

            this.listenerSocket = new WebSocketServer( wsOptions );
        ...
        {
            this.logger.error( "Error al abrir el WebSocket server:", err );
            throw err;
        }
    }
    close ()
    ...
    verifyClient ( info, response )
    {
        ...
        if ( Array.isArray( this.settings.listenerForbiddenIPs ) && this.settings.listenerForbiddenIPs.indexOf( address ) !== -1 )
        {
